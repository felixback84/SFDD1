    ...
    db
        ...
        .then((doc) => {
            let deviceDataFilter = {
                ...
                // post in payu
                // var to hold response of payu
                let resJson = {}; 
                // declarate the function to post in payu
                async function postToPayUAndFirebase(allDataToPostInPayU) {
                ...
                // run it
                postToPayUAndFirebase(allDataToPostInPayU).catch(console.error);
                // send response to client with data for web app cor¡nfirm payment
                if(resJson.status == 200){
...
exports.postDataCheckOutAdventure = (req, res) => {
    ...
    // ask for device info
    db
        .doc(`/adventure/${req.params.adventureId}`)
        .get()
        .then((doc) => {
