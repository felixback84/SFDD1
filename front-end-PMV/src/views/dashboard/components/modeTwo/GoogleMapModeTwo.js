...
class GoogleMapModeTwo extends Component {
	...
	handleApiLoaded = (map, maps, data) => {
        ...
        // top5Tags or top5Tag data
        // const dataTags = data.top5Tag.idOfSpecifiStaticDevices.length == 0 ? (data.top5Tags):(data.top5Tag)

        // var to fill the data passed
        let dataTags = []
        // check the data passed
        if(data.hasOwnProperty('top5TagsListener')) {
            dataTags = data.top5TagsListener
        } else if(data.hasOwnProperty('top5TagListener')){
            dataTags = data.top5TagListener
        }
