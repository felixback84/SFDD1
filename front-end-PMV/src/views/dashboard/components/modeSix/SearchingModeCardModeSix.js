...
import SearchingModeSwitcherFour from "./SearchingModeSwitcherSix"
// Redux stuff
import { connect } from 'react-redux'

// cards
const SearchingModeCardModeSix = (props) => {
  // styles
				...
				<CardStats
					...
					title={
						...
						(
							...
							data.top5ProductsListener.length != 0  && props.searchingMode === "modeSix" ?
					...
					color={
						props.searchingMode === "modeSix" && colorClass.colorPicker(data.color)
					} 
					footer={
						<>
							<Box
							...
							>
								The closer product to you is: {
									data.top5Products.length !== 0 && props.searchingMode === "modeSix" ? 
									(data.top5Products[0].meters.toFixed(2)):("")
								}
							</Box>
							...
							>
								{/* number of items */}
								You match with {props.searchingMode === "modeSix" && data.top5Products.length} products
							</Box>
						</>
					}
				/>		
...
export default connect(mapStateToProps)(SearchingModeCardModeSix)